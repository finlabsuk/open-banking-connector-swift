...
// MARK:- Transaction type protocols

protocol OBAIReadTransactionDataProtocol: OBAIReadResourceDataProtocol {
    var transaction: [OBAIResourceType]? { get }
}
extension OBAIReadTransactionDataProtocol {
    var resource: [OBAIResourceType]? { return transaction }
}

protocol OBAITransactionProtocol: OBAIResourceProtocol {
    associatedtype CreditDebitIndicatorType: RawRepresentableWithStringRawValue
