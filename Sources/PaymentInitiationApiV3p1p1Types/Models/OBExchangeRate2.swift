//
// OBExchangeRate2.swift
//
// Generated by swagger-codegen
// https://github.com/swagger-api/swagger-codegen
//

import Foundation


/** Further detailed information on the exchange rate that has been used in the payment transaction. */

public struct OBExchangeRate2: Codable {

    /** Currency in which the rate of exchange is expressed in a currency exchange. In the example 1GBP &#x3D; xxxCUR, the unit currency is GBP. */
    public var unitCurrency: String
    /** The factor used for conversion of an amount from one currency to another. This reflects the price at which one currency was bought with another currency. */
    public var exchangeRate: Double
    public var rateType: OBExchangeRateType2Code
    /** Unique and unambiguous reference to the foreign exchange contract agreed between the initiating party/creditor and the debtor agent. */
    public var contractIdentification: String?
    /** Specified date and time the exchange rate agreement will expire. All dates in the JSON payloads are represented in ISO 8601 date-time format.  All date-time fields in responses must include the timezone. An example is below: 2017-04-05T10:43:07+00:00 */
    public var expirationDateTime: Date?

    public init(unitCurrency: String, exchangeRate: Double, rateType: OBExchangeRateType2Code, contractIdentification: String?, expirationDateTime: Date?) {
        self.unitCurrency = unitCurrency
        self.exchangeRate = exchangeRate
        self.rateType = rateType
        self.contractIdentification = contractIdentification
        self.expirationDateTime = expirationDateTime
    }

    public enum CodingKeys: String, CodingKey { 
        case unitCurrency = "UnitCurrency"
        case exchangeRate = "ExchangeRate"
        case rateType = "RateType"
        case contractIdentification = "ContractIdentification"
        case expirationDateTime = "ExpirationDateTime"
    }


}

